...
(function () {
  ...
  OneSlider.register = function (name, factory) {
    var module = { name: name, factory: factory };
    modules.push(module);
    if (booted) {
      try { factory(OneSlider); }
      catch (e) { if (window.console) console.warn('[OneSlider]', name, e); }
    }
  };
  ...
  // Palette overlays are disabled. The active palette is owned by
  // oneslider-core.css so page-specific layouts do not get a second palette
  // stylesheet injected on top of their own CSS.
  // ====================================================================
  (function initSystemPalette() {
    ...
    function clearPaletteOverlay() {
      removeToolbar();
      var link = document.getElementById('os-palette-preview-css');
      if (link && link.parentNode) link.parentNode.removeChild(link);
      OneSlider.palettePreview = 'current';
      OneSlider.paletteMode = 'light';
      return 'current';
    }
    ...
    clearPaletteOverlay();
    OneSlider.setPalettePreview = clearPaletteOverlay;
    OneSlider.availablePalettePreviews = ['current'];
  })();
  ...
  // ====================================================================
  // Module: continentCountryTable
  // Adds group filtering and sortable metric columns to continent country
  // comparison tables generated by scripts/build/build-continents.py.
  // ====================================================================
  OneSlider.register('continentCountryTable', function () {
    var tables = document.querySelectorAll('[data-continent-country-table]');
    ...
    function setSort(table, button) {
      var headers = Array.prototype.slice.call(table.querySelectorAll('[data-sort-key]'));
      var tbody = table.tBodies[0];
      if (!tbody) return;
      var index = headers.indexOf(button);
      if (index < 0) return;
      var numeric = button.getAttribute('data-sort-type') === 'number';
      var current = button.getAttribute('aria-sort') || 'none';
      var direction = current === 'descending' ? 'ascending' : 'descending';
      ...
      var rows = Array.prototype.slice.call(tbody.querySelectorAll('[data-country-row]'));
      rows.sort(function (a, b) {
        var av = cellValue(a, index, numeric);
        var bv = cellValue(b, index, numeric);
        if (numeric) {
          var aMissing = av <= -9999;
          var bMissing = bv <= -9999;
          if (aMissing && !bMissing) return 1;
          if (!aMissing && bMissing) return -1;
          if (aMissing && bMissing) return 0;
          return direction === 'ascending' ? av - bv : bv - av;
        }
        return direction === 'ascending' ? String(av).localeCompare(String(bv)) : String(bv).localeCompare(String(av));
      });
      rows.forEach(function (row) { tbody.appendChild(row); });
    }

    function setFilter(table, value) {
      var rows = table.querySelectorAll('[data-country-row]');
      rows.forEach(function (row) {
        var group = row.getAttribute('data-group') || '';
        row.hidden = value !== 'all' && group !== value;
      });
    }

    tables.forEach(function (table) {
      table.querySelectorAll('[data-sort-key]').forEach(function (button) {
        button.addEventListener('click', function () { setSort(table, button); });
      });

      var tools = table.closest('.continent-country-overview')?.querySelector('[data-continent-table-tools]');
      if (!tools) return;
      tools.querySelectorAll('[data-continent-filter]').forEach(function (button) {
        button.addEventListener('click', function () {
          var value = button.getAttribute('data-continent-filter') || 'all';
          tools.querySelectorAll('[data-continent-filter]').forEach(function (item) {
