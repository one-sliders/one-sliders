...
function enrichmentHtml(page) {
  ...
  if (eventLike) {
    return `<section class="content-note" aria-label="${escapeAttr(name)} summary"><h2>${escapeAttr(name)} overview</h2><p>${escapeAttr(name)} belongs in the ${escapeAttr(context)} collection on OneSliders. Use this page as the compact evergreen entry point: current edition details, location cues, schedule context and related topic links are grouped into the page components. The layout is built around cards, rankings, timelines, calendars and result blocks rather than long-form article text.</p><p>The page should stand on concrete event data, visual components and linked context.</p></section>`;
  }
  return `<section class="content-note" aria-label="${escapeAttr(name)} summary"><h2>${escapeAttr(name)} on OneSliders</h2><p>${escapeAttr(name)} is the browsing page for ${escapeAttr(context)}. It groups related OneSliders entries so visitors can move from a broad theme to specific events, places and practical planning details without opening a long article. The focus is visual orientation: event cards, calendars, location links, rankings and comparison blocks.</p><p>The page is intentionally concise, but it should still stand on its own in search. The page should stand on event cards, location links, calendars and comparison blocks.</p></section>`;
}
