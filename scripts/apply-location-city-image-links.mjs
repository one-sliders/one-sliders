...
function ensureImagePair(pageFile, slug, continentSlug, countrySlug = '', options = {}) {
  ...
  const script = [
    ...
    'if (not mini.exists()) or mini.stat().st_size == 0:',
    '    crop_resize(im, (400, 300)).save(mini, optimize=True)',
  ].join('\n');
  execFileSync(python, ['-c', script, pair.source, pair.hero, pair.mini], { stdio: 'inherit' });
  return {
...
function ensureCityImagePair(cityFile, citySlug, continentSlug, countrySlug) {
  const pair = ensureImagePair(cityFile, citySlug, continentSlug, countrySlug, { strictLocal: true });
...
const files = walk(locationsRoot).filter((file) => file.endsWith('.html'));
let countries = 0;
let cities = 0;
